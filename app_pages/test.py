...
def render():
    ...
    if dividend_mode == "同敲出票息":
        tmp = [float(x.rstrip("%"))/100 for x in obs_coupons_input.replace("\n",",").split(",") if x.strip()]
    ...
    def parse_pct_list(s):
        return [float(x.rstrip("%"))/100 for x in s.replace("\n",",").split(",") if x.strip()]
    ...
    fig1 = go.Figure()
    fig1.add_trace(go.Scatter(x=x_perc, y=payoff_knockin, mode="lines", name="敲入已触发", line=dict(color="red")))
    fig1.add_trace(go.Scatter(x=x_perc, y=payoff_no_knockin, mode="lines", name="未发生敲入", line=dict(color="green", dash="dash")))
    fig1.add_vline(x=knock_in_pct, line_dash="dot", line_color="red", annotation_text=f"敲入 {knock_in_pct:.1f}%", annotation_position="bottom left")
    fig1.add_vline(x=100, line_dash="dot", line_color="gray", annotation_text="期初价 100%", annotation_position="bottom right")
    fig1.update_layout(title="理论收益曲线", xaxis_title="标的最终(% )", yaxis_title="收益%(相对保证金)", template="plotly_white")
    ...
    raw          = get_price_data([underlying_code], sim_start_date.strftime("%Y-%m-%d"), fetch_end.strftime("%Y-%m-%d"))
    ...
    fig2.add_trace(go.Scatter(x=[sim_df.index[0],sim_df.index[-1]], y=[knock_in_level]*2,
                              mode="lines", name="敲入线", line=dict(color="red", dash="dash")))
    xs,ys=[],[]
    ...
    if xs:
        fig2.add_trace(go.Scatter(x=xs,y=ys,mode="markers",name="敲出障碍价",marker=dict(color="green",size=8)))
    if knock_in_date:
        fig2.add_vline(x=knock_in_date,line_dash="dot",line_color="red")
        fig2.add_annotation(x=knock_in_date,y=max(sim_prices),text="敲入",showarrow=True,arrowhead=1,font=dict(color="red"))
    if knock_out_date:
        fig2.add_vline(x=knock_out_date,line_dash="dot",line_color="green")
        fig2.add_annotation(x=knock_out_date,y=max(sim_prices),text="敲出",showarrow=True,arrowhead=1,font=dict(color="green"))
    ...
    if knock_out_date:
        ...
        st.write(
            ...
            f"- 存续工作日：{active_days} 天  \n"
            f"- 年化票息：{coupon*100:.2f}%  \n"
            f"- 收益：{payoff:.2f} 万元（含本金）"
        )
    elif knock_ined:
        # 敲入发生但未敲出
